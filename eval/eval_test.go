...
func TestFunctionObject(t *testing.T) {
	...
	have := testEval(input)
	fn, ok := have.(*objects.Function)
	if !ok {
		t.Fatalf("object is not Function. have = %T (%+v)", have, have)
	}

	if len(fn.Parameters) != 1 {
		t.Fatalf("functions has wrong parameters. Parameters=%+v", fn.Parameters)
	}

	if fn.Parameters[0].String() != "x" {
		t.Fatalf("paramater is not 'x'. have=%q", fn.Parameters[0])
	}

	expectedBody := "(x + 2)"

	if fn.Body.String() != expectedBody {
		t.Fatalf("body is not %q. have %q", expectedBody, fn.Body.String())
	}
}
...
if (10 > 1) {
	...
	for _, tt := range tests {
		...
		err, ok := have.(*objects.Error)
		if !ok {
			t.Errorf("no error object returned. have=%T (%+v)", have, have)
			continue
		}

		if err.Value != tt.expected {
			t.Errorf("wrong error message. expected %q, have %q", tt.expected, err.Value)
		}
	}
}
...
func TestIfElseExpressions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if ok {
			testIntegerObject(t, have, int64(i))
		} else {
			testNullObject(t, have)
		}
	}
}
...
func testNullObject(t *testing.T, o objects.Object) bool {
	if o != NULL {
		t.Errorf("object is not NULL. have=%T (%+v)", o, o)
		return false
	}
